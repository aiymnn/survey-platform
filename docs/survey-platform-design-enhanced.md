...
# Appendix: Advanced LimeSurvey Parity Documentation

The following split documents extend this main design with deeper LimeSurvey-like architecture modules. They are intentionally separated from the main file so developers can reference one subsystem at a time during implementation.

| Document | Purpose |
|---|---|
| `00-index.md` | Documentation index and recommended reading order. |
| `01-expression-manager.md` | Advanced relevance, validation, equation, piping, parser, AST, and dependency graph design. |
| `02-multilingual-localization.md` | Survey language, translation tables, fallback logic, localized runtime, email, and export. |
| `03-question-type-mapping-limesurvey.md` | Detailed LimeSurvey-like question type registry, mapping, answer shape, reporting, and export columns. |
| `04-permissions-rbac-granular.md` | Granular role and permission model for organization, survey, response, participant, plugin, and system scopes. |
| `05-theme-template-engine.md` | Theme resolution, template engine, renderer overrides, branding, white-label, and custom CSS rules. |
| `06-plugin-lifecycle.md` | Plugin lifecycle, manifest, install/activate/configure hooks, navigation, permissions, and failure handling. |
| `07-email-token-workflow.md` | Invitation, reminder, confirmation, token hashing, email logs, queue workflow, and localized templates. |
| `08-import-export-compatibility.md` | `.lss`, `.lsa`, `.lsq`, CSV, Excel, JSON, PDF compatibility and import/export pipelines. |
| `09-statistics-engine.md` | Aggregation, question summaries, filters, cross-tabs, trends, snapshots, and AI analytics integration. |
| `10-admin-system-config.md` | Global admin settings, SMTP, storage, queue, cache, security, maintenance, feature flags, and health checks. |
| `11-development-roadmap.md` | Suggested implementation order from MVP to LimeSurvey-like parity. |

## Practical Development Note

Use the main file as the overall system design. Use the split documents as module-specific implementation references. This keeps the documentation easier to maintain while still covering advanced LimeSurvey-like features.
